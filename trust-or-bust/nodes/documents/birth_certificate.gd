...
@export var human: Character:
	...
	set(value):
		%BCName.text = value.name
		%BCDOB.text = %BCDOB.text.replacen("!MONTH", str(value.dob.month))
		%BCDOB.text = %BCDOB.text.replacen("!DAY", str(value.dob.day))
		%BCDOB.text = %BCDOB.text.replacen("!YEAR", str(value.dob.year))
		%Parent1.text = %Parent1.text.replacen("!PARENT1", str(value.parents[0]))
		if value.parents.size() <= 1 && value.is_child:
			value.parents.append("Beff Jezos")
		elif value.parents.size() <= 1:
			value.parents.append("Play CeHolder")
		%Parent2.text = %Parent2.text.replacen("!PARENT2", str(value.parents[1]))
