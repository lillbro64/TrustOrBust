...
@onready var rich_text: RichTextLabel = get_tree().root.find_child("RichTextLabel", true, false)
...
func init_character(): 
	if chara.visible:
		...
		rich_text.writte(characters[current_character].name + " ENTERS THE ROOM")
		await chara.anim_complete
		anim_complete.emit()
